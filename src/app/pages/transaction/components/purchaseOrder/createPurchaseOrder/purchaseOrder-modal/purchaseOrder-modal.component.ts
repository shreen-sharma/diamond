...
@Component({
  ...
  styleUrls: [('./purchaseOrder-modal.component.scss')],
  templateUrl: './purchaseOrder-modal.component.html'
})

export class PurchaseOrderModal implements OnInit {
  ...
  onChangeOrdNo(orderid: any) {
    ...
    if (!this.estimatedSizeFlag) {
      this.service.getAllLocalPurchaseOrderStockItemsReportLotwise(orderid).subscribe((reportList: any[]) => {
      ...
      this.mySettings = this.prepareSettingSize();
      this.service.localPurchaseOrderEstimateDistinctSizeReport(Number(orderid)).subscribe((reportList: any[]) => {
        ...
        this.service.localPurchaseOrderEstimateSizeReport(Number(orderid)).subscribe((reportList: any[]) => {

...
import { LocalSalesSupplierInvoiceReportService } from './localSalesSupplierInvoiceReport.service';
...
@Component({
  selector: 'localSalesSupplierInvoiceReport',
  templateUrl: './localSalesSupplierInvoiceReport.html',
  styleUrls: ['./localSalesSupplierInvoiceReport.scss'],

})
export class LocalSalesSupplierInvoiceReport implements OnInit {
  ...
  error: string = null;
  isLoading = false;
  localSalesSupplierInvoiceReportForm: FormGroup;
  ...
  constructor(private router: Router,
    ...
    private service: LocalSalesSupplierInvoiceReportService,
    ...
    // spinnerService.show();
    this.service.getConsolidatedLocalSalesInvoiceReport(-1, -1, -1).subscribe((tabList) => {
  ...
  ngOnInit() {
    this.settings = this.prepareSetting();
    this.reportHeading = "Local Sales Customer Report";
  }
  ...
  onUserRowSelect(event: any) {
    console.log(event);
    this.router.navigate(['../LSPartyInvoiceReport', event.data.partyId], { relativeTo: this.route });
  }
  ...
  ConvertToCSV() {
    ...
    new Angular2Csv(this.invoiceList, 'Local Sales - Supplier', options);
  ...
  createReport() {
    ...
    this.resetTotalCount();
    this.loading = true;
    this.service.getConsolidatedLocalSalesInvoiceReport(this.supplier.value == "" ? -1 : this.supplier.value,
  ...
  get ttlAmout(): number {
    return parseFloat(this.getStringColTotal('amount', '$').toFixed(2));
  }
  ...
  get amtPer(): number {
    ...
    this.localSalesSupplierInvoiceReportForm.markAsPristine();
  }

  private createForm() {
    this.localSalesSupplierInvoiceReportForm = this.fb.group({
    ...
    this.month = this.localSalesSupplierInvoiceReportForm.controls['month'];
    this.year = this.localSalesSupplierInvoiceReportForm.controls['year'];
    this.supplier = this.localSalesSupplierInvoiceReportForm.controls['supplier'];
  }
}
